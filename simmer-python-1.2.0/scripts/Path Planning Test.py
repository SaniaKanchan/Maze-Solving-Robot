...
# --- Enforce constraint: block the top-left NxN cells so robot never passes through them ---
TOP_LEFT_BLOCKS = 2  # change to 1,2,... to control how many top-left blocks to block (2 => 4 cells)
for ry in range(TOP_LEFT_BLOCKS):
    for cx in range(TOP_LEFT_BLOCKS):
        if 0 <= ry < ROWS and 0 <= cx < COLS:
            MAZE[ry][cx] = 1

# Cell size and movement parameters (all in inches)
CELL_SIZE_INCHES = 12  # Each maze cell is 12 inches (center to center)
MOVE_INCREMENT = 12    # Move in 12-inch increments
...
def execute_with_relocalization(commands):
    ...
    for i, cmd in enumerate(commands, 1):
        ...
        if i < len(commands):
            next_cmd = commands[i]
            input(f"  Next command: {next_cmd} - Press Enter to proceed...")
...
if __name__ == "__main__":
    ...
    start_cell = (5, 1)  # col 7, row 3
    goal_cell = (0, 2)   # col 0, row 2
    start_orientation = 270  # 0 is right, 90 is down, 180 is left, 270 is up
