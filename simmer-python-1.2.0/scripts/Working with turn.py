    ...
    def localize_with_movement(self, max_samples=50):
        ...
        for sample in range(max_samples):
            ...
            if sample < max_samples - 1:
                if relative_walls['front']:
                    # Wall ahead - need to turn
                    turn_direction = self.find_turn_direction(relative_walls)
                    turn_degrees = self.execute_turn(turn_direction)
                    
                    # Update all candidate histories with the turn
                    for i in range(len(all_candidates)):
                        all_candidates[i] = self.update_candidates_after_turn(
                            all_candidates[i], turn_degrees
                        )
                else:
                    ...
                    # Update all candidate histories with the move
                    for i in range(len(all_candidates)):
